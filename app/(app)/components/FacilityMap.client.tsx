...
import { Layers, Box, Maximize2, Compass } from 'lucide-react';
...
export default function FacilityMap({ facilities, selectedFacilityId, userCoords, onSelectFacility }: FacilityMapProps) {
  ...
  useEffect(() => {
    ...
    if (userCoords) {
      L.circle([userCoords.lat, userCoords.lng], {
        ...
        fillColor: '#7C3AED',
        fillOpacity: 0.1,
        radius: 5000 // 5km proximity radius
      }).addTo(map);
      ...
      const userMarker = L.marker([userCoords.lat, userCoords.lng], { icon: userDivIcon }).addTo(map);
    ...
    // Render PHC Markers with 3D Distance Badges
    facilities.forEach((f) => {
      ...
      const marker = L.marker([f.latitude, f.longitude], { icon: facilityDivIcon }).addTo(map);
      ...
      marker.bindPopup(`
        <div style="font-family: system-ui; max-width: 220px; padding: 4px;">
          ...
          <div style="background: #FAF6EE; padding: 6px; border-radius: 8px; margin-bottom: 6px; border: 1px solid #E5DCC8;">
            <p style="margin: 0; font-size: 12px; color: #D85A30; font-weight: 800;">📍 Accurate Distance: ${f.distance_km} km</p>
  ...
  }, [facilities, selectedFacilityId, userCoords, onSelectFacility]);
  ...
  return (
    <div className="relative w-full h-full min-h-[440px] map-3d-wrapper">
      ...
      <div className="absolute bottom-4 left-4 z-20 bg-white/90 backdrop-blur-md border border-[#E5DCC8] px-3 py-1.5 rounded-xl text-[11px] font-bold text-[#2C2418] shadow-md flex items-center gap-2">
        ...
        <span>Accurate GPS Distance Engine</span>
      </div>
